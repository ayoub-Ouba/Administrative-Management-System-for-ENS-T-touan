...
var fe = class {
  constructor() {
    ...
    this.create = (n) => {
      var A;
      let _a = n, { message: t } = _a, a = __objRest(_a, ["message"]), g = typeof (n == null ? void 0 : n.id) == "number" || ((A = n.id) == null ? void 0 : A.length) > 0 ? n.id : ue++, E = this.toasts.find((T) => T.id === g), y = n.dismissible === void 0 ? true : n.dismissible;
      return E ? this.toasts = this.toasts.map((T) => T.id === g ? (this.publish(__spreadProps(__spreadValues(__spreadValues({}, T), n), { id: g, title: t })), __spreadProps(__spreadValues(__spreadValues({}, T), n), { id: g, dismissible: y, title: t })) : T) : this.addToast(__spreadProps(__spreadValues({ title: t }, a), { dismissible: y, id: g })), g;
    };
    ...
    this.message = (n, t) => this.create(__spreadProps(__spreadValues({}, t), { message: n }));
    this.error = (n, t) => this.create(__spreadProps(__spreadValues({}, t), { message: n, type: "error" }));
    this.success = (n, t) => this.create(__spreadProps(__spreadValues({}, t), { type: "success", message: n }));
    this.info = (n, t) => this.create(__spreadProps(__spreadValues({}, t), { type: "info", message: n }));
    this.warning = (n, t) => this.create(__spreadProps(__spreadValues({}, t), { type: "warning", message: n }));
    this.loading = (n, t) => this.create(__spreadProps(__spreadValues({}, t), { type: "loading", message: n }));
    this.promise = (n, t) => {
      if (!t)
        return;
      let a;
      t.loading !== void 0 && (a = this.create(__spreadProps(__spreadValues({}, t), { promise: n, type: "loading", message: t.loading, description: typeof t.description != "function" ? t.description : void 0 })));
    ...
    this.custom = (n, t) => {
      ...
      return this.create(__spreadValues({ jsx: n(a), id: a }, t)), a;
    };
...
var xn = (r, n) => {
  ...
  return K.addToast(__spreadProps(__spreadValues({ title: r }, n), { id: t })), t;
};
var vn = xn;
...
var Pn = (r) => {
  ...
  Z.current = import_react.default.useMemo(() => G * it + Pt, [G, Pt]), import_react.default.useEffect(() => {
    ...
    c.style.height = _, D(S), E(($) => $.find((h) => h.toastId === t.id) ? $.map((h) => h.toastId === t.id ? __spreadProps(__spreadValues({}, h), { height: S }) : h) : [{ toastId: t.id, height: S, position: t.position }, ...$]);
...
var Jn = (r) => {
  ...
  return import_react.default.useEffect(() => K.subscribe((b) => {
    if (b.dismiss) {
      lt((u) => u.map((D) => D.id === b.id ? __spreadProps(__spreadValues({}, D), { delete: true }) : D));
      return;
    }
    setTimeout(() => {
      import_react_dom.default.flushSync(() => {
        lt((u) => {
          ...
          return D !== -1 ? [...u.slice(0, D), __spreadValues(__spreadValues({}, u[D]), b), ...u.slice(D + 1)] : [b, ...u];
        });
      });
    });
  }), []), import_react.default.useEffect(() => {
    ...
    var C;
    let [D, p] = b.split("-");
    return import_react.default.createElement("ol", { key: b, dir: st === "auto" ? $e() : st, tabIndex: -1, ref: et, className: y, "data-sonner-toaster": true, "data-theme": Lt, "data-rich-colors": mt, "data-y-position": D, "data-x-position": p, style: __spreadValues({ "--front-toast-height": `${((C = z[0]) == null ? void 0 : C.height) || 0}px`, "--offset": typeof A == "number" ? `${A}px` : A || Rn, "--width": `${Dn}px`, "--gap": `${Ct}px` }, Dt), onBlur: (w) => {
