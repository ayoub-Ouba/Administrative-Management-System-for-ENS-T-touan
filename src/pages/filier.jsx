...
export default function Filiers(){
    const  [filiers, setfiliers] = useState([]);
    const  [loding, setloding] = useState(true);
   
    useEffect(() => {
        axios.get('http://localhost:8082/projet/api/Filiere').then(
        (res2)=>{ setfiliers(res2.data);if(Array.isArray(res2.data)){setloding(false)}}
        ) 
    }, []);
//logique pour pagination 
    ...
    var currentetud=filiers.slice(indexoffirstetud,indexoflasteetud)
    const pagenumber=[]
    ...
    for(let i=1;i<Math.ceil(filiers.length/number_etud)+1;i++){
        pagenumber.push(i);
    }
//search
...
// const [msg,setmsg]=useState("");
    ...
    <div>
      <h2 className="font-semibold text-2xl text-gray-600 mt-10 ">{ 'Filiéres'}</h2>
      ...
      <div className="bg-white rounded shadow-lg py-10 px-4 md:p-[1rem] mb-6 h-[600px]">
        <div className="grid gap-4 gap-y-2 text-sm grid-cols-1 lg:grid-cols-1">
          <div className="text-gray-600">
            <p className="font-medium text-lg mb-10">Liste des Filiéres  </p>
          </div>
